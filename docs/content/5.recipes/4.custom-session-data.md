...
This guide explains how to add custom data to the user session, in this example we will continue to use the [Strapi Recipe](/nuxt-auth/recipes/strapi)  but it can easily be applied to any OAuth provider.
      ...
      async authorize(credentials: any) {
        ...
        if (response.user) {
          const u = {
            ...
            access_token: response.jwt, // additional field
...
Please Notice that with this approach there is no need to use profile function or override the types next-auth.
