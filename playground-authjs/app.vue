...
<template>
  <div>
    ...
    <p>
      See all available authentication & session information below. Navigate to
      different sub-pages to test out the app.
    </p>
    ...
    <pre>
Decoded JWT token: {{
        token?.token || 'no token present, are you logged in?'
      }}</pre
    >
    ...
    <div>
      ...
      <br />
      <button @click="getSession({ required: false })">refresh session</button>
    </div>
    <hr />
