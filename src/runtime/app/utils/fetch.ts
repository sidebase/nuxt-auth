...
import { ERROR_PREFIX } from '../../shared/logger'
...
export async function _fetch<T>(
  ...
  try {
    ...
    if (runtimeConfig.public.auth.provider.type === 'authjs') {
      errorMessage +=
        ' Have you added the authentication handler server-endpoint `[...].ts`? Have you added the authentication handler in a non-default location (default is `~/server/api/auth/[...].ts`) and not updated the module-setting `auth.basePath`?'
    }
