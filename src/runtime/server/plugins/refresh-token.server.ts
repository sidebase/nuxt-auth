...
export default defineNuxtPlugin({
  ...
  async setup (nuxtApp) {
    ...
    if (refreshToken.value) {
      ...
      const extractedToken = jsonPointerGet(response, config.provider.token.signInResponseTokenPointer)
      ...
      const extractedRefreshToken = jsonPointerGet(response, config.provider.refreshToken.signInResponseRefreshTokenPointer)
