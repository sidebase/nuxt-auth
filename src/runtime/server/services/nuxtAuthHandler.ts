 ...
 */
const parseActionAndProvider = ({
  context,
}: H3Event): { action: AuthAction; providerId: string | undefined } => {
  ...
  const action = SUPPORTED_ACTIONS.find(
    (action) => action === unvalidatedAction
  );
  if (!action) {
    throw createError({
      statusCode: 400,
...
export const NuxtAuthHandler = (nuxtAuthOptions?: AuthOptions) => {
   ...
   */
  const getInternalNextAuthRequestData = async (
    event: H3Event
  ): Promise<RequestInternal> => {
    const nextRequest: Omit<RequestInternal, 'action'> = {
      host: detectHost(event, {
        trusted: useRuntimeConfig().auth.trustHost,
        basePath: useRuntimeConfig().auth.basePath,
      }),
      body: undefined,
...
export const getServerSession = async (event: H3Event) => {
  ...
  if (!preparedAuthHandler) {
    ...
    await $fetch(joinURL(authBasePath, '/session')).catch(
      (error) => error.data
    );
    if (!preparedAuthHandler) {
      throw createError({
...
export const getToken = ({
  ...
  secret,
  ...rest
}: Omit<GetTokenParams, 'req'> & { event: H3Event }) =>
  nextGetToken({
