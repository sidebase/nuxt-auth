...
export function generateModuleTypes (provider: AuthProviders) {
  ...
  if (provider.type === 'refresh') {
    providerSpecificTypes.push(`  const { getServerSession, getToken, getRefreshToken }: typeof import('${resolve('./runtime/server/services/refresh')}')`)
    providerSpecificTypes.push(genInterface('SessionData', (provider as any).session.dataType))
  }
